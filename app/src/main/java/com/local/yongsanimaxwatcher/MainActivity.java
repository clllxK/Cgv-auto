...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        super.onCreate(b);prefs=new Prefs(this);kakao=new KakaoClient(this);new NotificationHelper(this);selectedMovie=prefs.getUiMovie();requestNotificationPermission();setContentView(buildUi());ui.post(statusLoop);ui.postDelayed(()->loadAll(false),200);
    }
    ...
    private View buildUi(){
        ...
        ScrollView scroll=new ScrollView(this);LinearLayout c=new LinearLayout(this);c.setOrientation(LinearLayout.VERTICAL);c.setPadding(dp(16),dp(14),dp(16),dp(100));
        c.addView(statusCard());c.addView(label("극장"),top(18));c.addView(theaterCard(),top(7));
        ...
        LinearLayout pairCard=card();adjacentOnly=new CheckBox(this);adjacentOnly.setText("👫 붙은 2자리만 알림");adjacentOnly.setTextSize(16);adjacentOnly.setTextColor(TEXT);adjacentOnly.setChecked(prefs.isAdjacentSeatOnly());
        adjacentOnly.setOnCheckedChangeListener((buttonView,isChecked)->{if(renderingAdjacent)return;stopForConfigChange(false);prefs.setAdjacentSeatOnly(isChecked);if(isChecked&&prefs.getSelectedShowtimeKeys().size()>1){prefs.clearSelectedShowtimes();toast("연석 감시는 한 회차씩 정확히 보게 돼. 원하는 시간 하나만 다시 골라줘.");renderTimes();}refreshAdjacentUi();refreshStatus();});
        pairCard.addView(adjacentOnly);adjacentHint=text("OFF: 기존처럼 잔여석 증가를 감지해.\nON: 선택한 한 회차를 앱이 백그라운드에서 자동으로 열고 같은 행의 연속 2자리를 30초마다 확인해.",12,false,SUB);adjacentHint.setLineSpacing(dp(2),1);pairCard.addView(adjacentHint,top(5));c.addView(pairCard,top(12));
    ...
    private void renderTimes(){timeList.removeAllViews();if(selectedMovie.isEmpty()||selectedDate.isEmpty()){timeList.addView(text("영화와 특별관을 선택하면 열린 회차가 여기에 보여.",14,false,SUB));return;}List<CgvClient.Showtime>list=filtered(schedules.get(selectedDate));if(list.isEmpty()){timeList.addView(text("이 날짜에는 해당 특별관 회차가 없어.",14,false,SUB));return;}timeList.addView(text(prefs.isAdjacentSeatOnly()?"시간 하나 선택 = 그 회차의 붙은 2자리 백그라운드 감시":"시간 버튼 = 해당 회차 취소표 알림",13,true,SUB));int idx=0;while(idx<list.size()){LinearLayout row=new LinearLayout(this);for(int col=0;col<3;col++){if(idx>=list.size()){row.addView(new View(this),new LinearLayout.LayoutParams(0,dp(1),1));continue;}CgvClient.Showtime s=list.get(idx++);boolean on=prefs.getSelectedShowtimeKeys().contains(s.key());String seat=s.freeSeats>=0?"\n"+s.freeSeats+"석":"";TextView b=text((on?"✓ ":"")+s.time+seat,14,true,on?Color.WHITE:TEXT);b.setGravity(Gravity.CENTER);b.setBackground(roundBg(on?RED:Color.WHITE,dp(10),on?RED:BORDER,1));b.setOnClickListener(v->toggleShowtime(s));LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(0,dp(60),1);if(col>0)lp.leftMargin=dp(7);row.addView(b,lp);}timeList.addView(row,top(7));}}

    private void refreshAdjacentUi(){if(adjacentOnly==null)return;renderingAdjacent=true;adjacentOnly.setChecked(prefs.isAdjacentSeatOnly());renderingAdjacent=false;if(adjacentHint!=null)adjacentHint.setText(prefs.isAdjacentSeatOnly()?"ON · 원하는 시간 하나만 고르고 감시 시작을 누르면 돼. 앱이 CGV 예매 흐름을 백그라운드에서 자동으로 진행하고 30초마다 실제 연속 2좌석을 확인해. 보안확인이 정말 필요한 경우에만 알림으로 알려줘.":"OFF · 기존처럼 선택한 회차의 잔여석 증가를 백그라운드에서 감지해.");}
    ...
    private void toggleShowtime(CgvClient.Showtime s){stopForConfigChange(true);Set<String>keys=prefs.getSelectedShowtimeKeys(),labels=prefs.getSelectedShowtimeLabels();String label=DateUtil.pretty(s.date)+" · "+s.title+" · "+s.format+" · "+s.time;if(keys.contains(s.key())){keys.remove(s.key());labels.remove(label);}else{if(prefs.isAdjacentSeatOnly()){keys.clear();labels.clear();}keys.add(s.key());labels.add(label);}prefs.setSelectedShowtimes(keys,labels);renderTimes();refreshStatus();}
    ...
    private void toggleWatch(){
        ...
        if(prefs.isAdjacentSeatOnly()){
            if(prefs.getSelectedShowtimeKeys().size()!=1){new AlertDialog.Builder(this).setTitle("연석 감시할 시간을 하나 골라줘").setMessage("붙은 2자리 감시는 원하는 상영시간 하나를 선택하면 앱이 나머지는 자동으로 처리해.").setPositiveButton("확인",null).show();return;}
            startPairWatch();
            if(!prefs.getNewDateMovies().isEmpty())startStandardWatch(false);
            return;
        }
        ...
        startStandardWatch(true);
    }

    private void startPairWatch(){Intent i=new Intent(this,PairWatchService.class);i.setAction(PairWatchService.ACTION_START);startForegroundService(i);prefs.setPairWatching(true);refreshStatus();toast("붙은 2자리 백그라운드 감시 시작했어.");}
    private void startStandardWatch(boolean toastIt){Intent i=new Intent(this,WatchService.class);i.setAction(WatchService.ACTION_START);startForegroundService(i);prefs.setStandardWatching(true);refreshStatus();if(toastIt)toast("감시 시작했어.");}
    private void stopWatch(){Intent a=new Intent(this,WatchService.class);a.setAction(WatchService.ACTION_STOP);startService(a);Intent b=new Intent(this,PairWatchService.class);b.setAction(PairWatchService.ACTION_STOP);startService(b);prefs.clearWatchingFlags();prefs.setStatus("정지됨");refreshStatus();}

    private void refreshStatus(){if(status==null)return;boolean w=prefs.isWatching(),pairW=prefs.isPairWatching();status.setText(pairW?"● 연석 감시 중":w?"● 감시 중":"정지");status.setTextColor(w?Color.WHITE:SUB);status.setBackground(roundBg(w?RED:Color.rgb(235,235,235),dp(20),Color.TRANSPARENT,0));StringBuilder b=new StringBuilder();if(!prefs.getNewDateMovies().isEmpty())b.append("🔔 ").append(android.text.TextUtils.join(", ",prefs.getNewDateMovies())).append(" · ").append(prefs.getFormatKeyword()).append(" 새 날짜");int n=prefs.getSelectedShowtimeKeys().size();if(n>0){if(b.length()>0)b.append("\n");b.append(prefs.isAdjacentSeatOnly()?"👫 붙은 2자리 감시 · "+n+"회차":"🎟 취소표 회차 "+n+"개");}if(b.length()==0)b.append("영화 → 특별관 → 알림을 선택해.");summary.setText(b.toString());startStop.setText(w?"감시 중지":"감시 시작");startStop.setBackground(roundBg(w?DARK:RED,dp(13),Color.TRANSPARENT,0));}
